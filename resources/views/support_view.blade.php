...
@section('content')
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h4 style="color:#A04000; margin-bottom:20px">Email Support Lists
                            <button class="btn btn-primary pull-right" data-toggle="modal" data-target="#user" type="submit">Add
                                Ticket</button>
                        </h4>
    ...
    <!-- ##################  ASSIGNED USER #########################  -->
    <div class="modal fade" id="assignedUserModal" tabindex="-1" role="dialog" aria-labelledby="assignedUserModalLabel">
        <div class="modal-dialog vertical-align-center modal_lg">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"><span
                ...
                <div class="modal-body">
                    <div class="modal-body">
                        <form id="assigned_user" action="javascript:void(0)" method="POST">
                            {!! csrf_field() !!}
                            ...
                            <button class="btn btn-primary mt-3" type="submit">
                                <i class="fa fa-fw fa-lg fa-check-circle" name="button"></i> Assign User
                            </button>
                        </form>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var BranchListTable = $('#branch_table').DataTable({
                ...
                "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                    ...
                    var assignedUserButton = '<td>' + aData['assigned'] + '  <br /> <button class="btn btn-info" data-toggle="modal" data-target="#assignedUserModal" data-id="' + aData['id'] + '" onclick="addAssignData(' + aData['id'] + ', '+aData['assigned_id'] + ');">Assign User</button></td>';
                    $('td:eq(9)', nRow).html(assignedUserButton);
        ...
        function addAssignData(ticket_id, assigned_id) {
            $('#assign_ticket_id').val(ticket_id);
            $('#assigned_user_id').val(assigned_id);
                    ...
                    if (result.success == 1) {
                        ...
                        Object.values(data).forEach(val => {
                            ...
                            add_row += '<div class="row">\
                                    <div class="col-md-3">\
                                    <p id="old_log">' + val.old_status_display + '</p>\
                                    </div>\
                                    <div class="col-md-3">\
                                      <p id="current_log">' + val.status_display + '</p>\
                                    </div>\
                                    <div class="col-md-3">\
                                       <p id="comment_log">' + val.comment + '</p>\
                                    </div>\
                                    <div class="col-md-3">\
                                      <p id="created_log">' + val.created_at + '</p>\
                                    </div>\
                            </div>';
        ...
        $(document).ready(function() {
            ...
            $("#assigned_user").validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: public_path + '/assign_user',
                        method: 'post',
                        data: new FormData($("#assigned_user")[0]),
                        dataType: 'json',
                        async: false,
                        cache: false,
                        processData: false,
