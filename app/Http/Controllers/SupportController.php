...
class SupportController extends Controller
{
    ...
    public function supportView()
    {
       ...
       ->select('name', 'id')
       ->where('role', 3)  // Filter users with role == 3
       ->where('status', 1)
       ->orderBy('name')
       ->get();
    ...
    public function assignUser(Request $request)
    {
        try {
            ...
            $request->validate([
                ...
                'assign_ticket_id' => 'required',
            ]);

            $assignedUserId = $request->input('assigned_user_id');
            $assign_ticket_id = $request->input('assign_ticket_id');

            $result = DB::table('support_tickets')->where('id', $assign_ticket_id)->update([
                'assigned' =>  $assignedUserId
            ]);

            return response()->json(['success' => 1, 'message' => 'User assigned successfully', 'assign_ticket_id' => $assign_ticket_id]);
    ...
    public function storeReplyTicket(Request $request)
    {
        ...
        $maildata["email"][] = env('MAIL_USERNAME');
